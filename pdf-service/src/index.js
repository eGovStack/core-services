 ...
 */
 const createPdfBinary=async(key,listDocDefinition,entityIds, formatconfig,successCallback, errorCallback,tenantId,starttime,totalobjectcount,userid) =>{
   try {
    ...
    else
    {
      ...
      await Promise.all([
      uploadFiles(dbInsertSingleRecords,dbInsertBulkRecords,formatconfig,listDocDefinition,key,false,jobid,noOfDefinitions,entityIds,starttime, successCallback, errorCallback,tenantId,totalobjectcount,userid),
      uploadFiles(dbInsertSingleRecords,dbInsertBulkRecords,formatconfig,listDocDefinition,key,true,jobid,noOfDefinitions,entityIds,starttime, successCallback, errorCallback,tenantId,totalobjectcount,userid)
      ]);
    }
  } catch (err) {
...
const uploadFiles=async(dbInsertSingleRecords,dbInsertBulkRecords,formatconfig,listDocDefinition,key,isconsolidated,jobid,noOfDefinitions,entityIds,starttime, successCallback, errorCallback,tenantId,totalobjectcount,userid)=>{
  ...
  convertedListDocDefinition.forEach(function(docDefinition, i)
  {
        ...
        const doc = printer.createPdfKitDocument(JSON.parse(JSON.stringify(docDefinition)));
        ...
        doc.on('data', function (chunk) {
          ...
          fileStoreAPICall(filename,tenantId,data).then((result)=>{
            ...
            if(!isconsolidated)
            {
              dbInsertSingleRecords.push({jobid,id:uuidv4(),createdby:userid,modifiedby:userid,entityid:entityIds[i],isconsolidated:false,filestoreids:[result],tenantId,createdtime:starttime,endtime:new Date().getTime(),totalcount:1});
            ...
            else if(isconsolidated &&(listOfFilestoreIds.length==noOfDefinitions))
            {
              ...
              dbInsertBulkRecords.push({jobid,id:uuidv4(),createdby:userid,modifiedby:userid,entityid:null,isconsolidated:true,filestoreids:listOfFilestoreIds,tenantId,createdtime:starttime,endtime:new Date().getTime(),totalcount:totalobjectcount});
            }
            if((dbInsertSingleRecords.length==totalobjectcount)&&(dbInsertBulkRecords.length==1)){

              insertStoreIds(dbInsertSingleRecords.concat(dbInsertBulkRecords),jobid,listOfFilestoreIds,tenantId,starttime,successCallback,errorCallback,totalobjectcount);
            }
            }).catch(err=>{
                logger.error(err.stack || err);
...
app.post("/pdf-service/v1/_create", asyncHandler(async (req, res)=> { 
  ...
  try{
   ...
   await createAndSave(req,res,response => {
...
// 
    ...
    if(valid)
    {
      let [formatConfigByFile,totalobjectcount,entityIds]=await prepareBegin(key,req,requestInfo,true,formatconfig,dataconfig);
...
export const createAndSave=async(req,res,successCallback,errorCallback)=>{
  var starttime=new Date().getTime();
...
// 
   ...
   if(valid)
   {
     let [formatConfigByFile,totalobjectcount,entityIds]=await prepareBegin(key,req,requestInfo,false,formatconfig,dataconfig);
     ...
     logger.info(`Applied templating engine on ${totalobjectcount} objects output will be in ${formatConfigByFile.length} files`);
     ...
     createPdfBinary(
       ...
       formatconfig,
       successCallback,
       errorCallback,tenantId,starttime,totalobjectcount,userid
     ).catch(err=>{
      logger.error(err.stack || err);
      errorCallback({message:"error occurred in createPdfBinary call: "+((typeof err)==='string')?err:err.message});
    });
   }
}
...
const prepareBegin=async(key,req,requestInfo,returnFileInResponse,formatconfig,dataconfig)=>{
  ...
  return await prepareBulk(key,dataconfig,formatconfig,req,baseKeyPath,requestInfo,returnFileInResponse,entityIdPath);
}

const handlelogic=async(key,formatObject,moduleObject,dataconfig,isCommonTableBorderRequired,requestInfo)=>{
...
const prepareBulk=async(key,dataconfig,formatconfig,req,baseKeyPath,requestInfo,returnFileInResponse,entityIdPath)=>{
  ...
  let moduleObjectsArray=checkifNullAndSetValue(jp.query(req.body||req,baseKeyPath),[],baseKeyPath);
    if(Array.isArray(moduleObjectsArray) && (moduleObjectsArray.length>0))
    {
      ...
      for(var i=0, len=moduleObjectsArray.length; i < len; i++)
      {
        ...
        let entityKey=checkifNullAndSetValue(jp.query(moduleObject,entityIdPath),[null],entityIdPath);
...
/////////////////////////////
        formatObject=await handlelogic(key,formatObject,moduleObject,dataconfig,isCommonTableBorderRequired,requestInfo);
