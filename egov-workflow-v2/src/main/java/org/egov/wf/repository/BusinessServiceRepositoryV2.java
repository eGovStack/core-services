...
import org.egov.wf.repository.querybuilder.BusinessServiceQueryBuilderV2;
import org.egov.wf.repository.rowmapper.BusinessServiceRowMapperV2;
import org.egov.wf.service.MDMSServiceV2;
...
@Slf4j
@Repository
public class BusinessServiceRepositoryV2 {


    private BusinessServiceQueryBuilderV2 queryBuilder;

    private JdbcTemplate jdbcTemplate;

    private BusinessServiceRowMapperV2 rowMapper;

    private WorkflowConfig config;

    private MDMSServiceV2 mdmsService;


    @Autowired
    public BusinessServiceRepositoryV2(BusinessServiceQueryBuilderV2 queryBuilder, JdbcTemplate jdbcTemplate,
                                     BusinessServiceRowMapperV2 rowMapper, WorkflowConfig config, MDMSServiceV2 mdmsService) {
    ...
    public List<BusinessService> getBusinessServices(BusinessServiceSearchCriteriaV2 criteria){
        String query;
        ...
        if(!CollectionUtils.isEmpty(stateLevelBusinessServices)){
            BusinessServiceSearchCriteriaV2 stateLevelCriteria = new BusinessServiceSearchCriteriaV2();
        ...
        if(!CollectionUtils.isEmpty(tenantBusinessServices)){
            BusinessServiceSearchCriteriaV2 tenantLevelCriteria = new BusinessServiceSearchCriteriaV2();
    ...
    private List<BusinessService> getAllBusinessService(){
        ...
        String query = queryBuilder.getBusinessServices(new BusinessServiceSearchCriteriaV2(), preparedStmtList);
